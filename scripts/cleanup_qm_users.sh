#!/usr/bin/env bash
# Delete residual qm-* Linux users left behind by quadletman.
#
# Usage:
...
#   sudo bash scripts/cleanup_qm_users.sh --dry-run    # list users, change nothing
#
...
if [[ ${#QM_USERS[@]} -eq 0 ]]; then
    echo "No qm-* users found."
    exit 0
fi
